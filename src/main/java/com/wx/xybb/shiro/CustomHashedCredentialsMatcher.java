...
public class CustomHashedCredentialsMatcher extends HashedCredentialsMatcher {
    ...
    public boolean doCredentialsMatch(AuthenticationToken token, AuthenticationInfo info) {
        ...
        /*
         * 判断token类型，1为web token，0为wx token
         * */
        String method = JwtTokenUtil.getMethod(accessToken);
        if(method.equals("0")){
            String jwtId = JwtTokenUtil.getJwtId(accessToken);
            /**
             * 判断token 是否主动登出
             */
            if (redisService.hasKey(Constant.JWT_ACCESS_TOKEN_BLACKLIST + jwtId)) {
