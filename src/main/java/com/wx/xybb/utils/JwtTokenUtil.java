...
public class JwtTokenUtil {
     ...
     * 获取标识符
     * */
    public static String getMethod(String token){
        String method = null;
        try {
            Claims claims =getClaimsFromToken(token);
            method = (String) claims.get(Constant.JWT_WX_MARK);
        }catch(Exception e){
            log.error("error={}",e);
        }
        return method;
    ...
    public static String getOpenid(String token){
        ...
        try {
            Claims claims =getClaimsFromToken(token);
            openid = (String) claims.get(Constant.JWT_WX_OPENID);
        }catch(Exception e){
            log.error("error={}",e);
        }
        return openid;
    ...
    public static String getStudentId(String token){
        ...
        try {
            Claims claims =getClaimsFromToken(token);
            studentId = (String) claims.get(Constant.JWT_WX_STUDENTID);
        }catch(Exception e){
            log.error("error={}",e);
        }
        return studentId;
    ...
    public static String getPassword(String token){
        ...
        try {
            Claims claims =getClaimsFromToken(token);
            password = (String) claims.get(Constant.JWT_WX_PASSWORD);
        }catch(Exception e){
            log.error("error={}",e);
        }
        return password;
